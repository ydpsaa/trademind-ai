    ...
    <section className="space-y-3">
      ...
      <div className="grid gap-4 xl:grid-cols-2">
        {providers.map((provider) => {
          const status = statuses.get(provider.provider) ?? {
            status: provider.defaultStatus,
            mode: provider.mode,
            label: formatStatus(provider.defaultStatus),
            lastCheckedAt: null,
            metadata: {},
          };
            ...
            <GlassCard key={provider.provider} className="p-4 md:p-5">
              <div className="flex flex-col gap-4 md:flex-row md:items-start md:justify-between">
                ...
                <div className="text-left text-xs text-zinc-500 md:text-right">
                  <div>
                    Mode <span className="capitalize text-zinc-300">{formatMode(status.mode)}</span>
                  </div>
                  <div className="mt-1">
                    Last checked <span className="text-zinc-300">{formatDateTime(status.lastCheckedAt)}</span>
                  </div>
                </div>
              </div>
...
export default async function ConnectionsPage() {
  ...
  const statuses = userConnectionProviders.map((provider) => runtimeStatus(provider, context.records, context));
  const statusMap = new Map(statuses.map((status, index) => [userConnectionProviders[index].provider, status]));
  const summary = summarizeConnections(statuses);
  const exchangeProviders = userConnectionProviders.filter((provider) => provider.category === "Exchange Accounts");
  const brokerProviders = userConnectionProviders.filter((provider) => provider.category === "Broker Accounts");
  const chartProviders = userConnectionProviders.filter((provider) => provider.category === "Charts & Market Data");
  ...
  return (
    <AppShell title="Connections" subtitle="Connect exchange, broker, charting, and market data tools for your trading workspace.">
      <div className="space-y-4">
        ...
        <ProviderSection title="Exchange Accounts" description="Read-only exchange import will power trade history, account analytics, and reconciliation." providers={exchangeProviders} statuses={statusMap} />
        <ProviderSection title="Broker Accounts" description="Broker bridge integrations for Forex, prop accounts, and journal sync." providers={brokerProviders} statuses={statusMap} />

...
export function RecentTradesCard({ className = "lg:col-span-6 2xl:col-span-5", trades, demoLabel = false }: RecentTradesCardProps) {
  ...
  return (
    <GlassCard className={`p-4 ${className}`}>
      ...
      <Link href={hasRealTrades ? "/journal" : "/journal/new"} className="mt-3 grid h-10 w-full place-items-center rounded-xl border border-white/10 bg-white/10 text-sm font-medium text-white transition hover:bg-white/15">
        {hasRealTrades ? "Go to Journal" : "Add Trade"}
      </Link>
    </GlassCard>
  );
}

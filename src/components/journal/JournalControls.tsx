...
export function JournalControls({ period, source, accounts, selectedAccount }: JournalControlsProps) {
  return (
    <GlassCard className="p-4">
      <div className="flex flex-col gap-4 xl:flex-row xl:items-center xl:justify-between">
        ...
        <div className="flex flex-wrap gap-2">
          <AccountSelector accounts={accounts} selectedAccount={selectedAccount} basePath="/journal" params={{ period, source }} />
      ...
      <div className="mt-5 grid gap-4 lg:grid-cols-[minmax(0,1fr)_auto] lg:items-center">
        <div className="flex flex-wrap gap-2 rounded-2xl border border-white/10 bg-black/20 p-1.5">
          {journalPeriods.map((item) => (
            <Link
              key={item.value}
              href={`/journal?period=${item.value}&source=${source}&account=${selectedAccount}`}
        ...
        <div className="flex flex-wrap gap-2">
          {sources.map((item) => (
            <Link
              key={item.value}
              href={`/journal?period=${period}&source=${item.value}&account=${selectedAccount}`}

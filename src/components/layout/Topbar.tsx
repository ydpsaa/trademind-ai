import { Bell, CalendarDays, ChevronDown, Menu, Sparkles } from "lucide-react";
...
export function Topbar({ title = "Dashboard", subtitle = "Welcome back, Alex. Here's your trading overview." }: TopbarProps) {
  return (
    <>
      ...
      <header className="flex flex-col gap-4 md:flex-row md:items-start md:justify-between">
        ...
        <div className="flex flex-wrap items-center gap-3">
          <button className="inline-flex h-11 items-center gap-3 rounded-xl border border-white/10 bg-white/[0.06] px-4 text-sm text-white shadow-inner shadow-white/5">
            All Accounts <ChevronDown className="h-4 w-4 text-zinc-400" />
          </button>
          <button className="inline-flex h-11 items-center gap-3 rounded-xl border border-white/10 bg-white/[0.06] px-4 text-sm text-white shadow-inner shadow-white/5">

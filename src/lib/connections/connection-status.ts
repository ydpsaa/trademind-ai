...
export const systemServiceProviders: ProviderCard[] = [
  {
    ...
    description: "Authentication, secure user data, data isolation, and future storage.",
    purpose: "Auth, secure data, storage",
    ...
    setupRequirements: ["Service endpoint", "Client configuration", "Active authenticated session"],
    safetyNotes: ["Sensitive server keys must stay server-side only.", "Data isolation remains enabled for user-owned data."],
  ...
  },
  {
    ...
    description: "AI Review generation with local AI or cloud AI support and local rules fallback.",
    ...
    actionLabel: "Test Status",
    setupRequirements: ["Local AI service or server AI key", "Optional model setting"],
    safetyNotes: ["AI keys are never exposed to client components.", "Fallback local rules remain available when no AI provider is configured."],
    roadmap: ["Additional AI provider options", "Cost controls", "Per-review generation logs"],
  },
  {
    provider: "economic-calendar",
    name: "Calendar Data",
    category: "Internal Data",
    description: "Platform-managed macro event data used for news risk context.",
    purpose: "News risk",
    powers: ["Calendar page", "Trade news context", "AI review news score"],
    mode: "configured",
    defaultStatus: "not_connected",
    actionLabel: "Test Status",
    setupRequirements: ["Calendar Data storage", "Authenticated read access", "Manual/sample events for now"],
...
export const userConnectionProviders: ProviderCard[] = [
  {
    ...
    description: "Connect a read-only account to import completed derivative trades into your journal. Connected accounts appear in the account selector.",
    purpose: "Trade import and account analytics",
    powers: ["Trade history import", "Account analytics", "Portfolio reconciliation", "Account selector"],
    mode: "read_only",
    defaultStatus: "not_connected",
    actionLabel: "Configure",
    setupRequirements: ["Read-only API key", "No withdrawal permission", "Unified or standard derivatives account"],
    safetyNotes: ["Only account verification and closed PnL GET endpoints are used.", "Trading execution is disabled and withdrawal permissions are rejected."],
    roadmap: ["Closed derivatives import", "Account selector entry", "Spot history import", "Incremental scheduled sync"],
  },
  {
    provider: "okx",
    name: "OKX",
    category: "Exchange Accounts",
    description: "Connect a read-only account to import completed margin and derivative trades into your journal. Connected accounts appear in the account selector.",
    purpose: "Trade import and account analytics",
    powers: ["Trade history import", "Account analytics", "Portfolio reconciliation", "Account selector"],
    mode: "read_only",
    defaultStatus: "not_connected",
    actionLabel: "Configure",
    setupRequirements: ["API key with Read permission only", "API secret and passphrase", "No Trade or Withdraw permission"],
    safetyNotes: ["Only account configuration and position history GET endpoints are used.", "Trading execution is disabled and write permissions are rejected."],
    roadmap: ["Closed margin and derivatives import", "Account selector entry", "Spot history import", "Incremental scheduled sync"],
  },
  {
    provider: "metatrader",
    name: "MetaTrader",
    ...
    powers: ["MT account import", "Prop account analytics", "Journal sync", "Account selector"],
    mode: "coming_soon",
    ...
    roadmap: ["Import bridge", "Account selector entry", "Trade screenshot sync", "Broker-specific adapters"],
  },
  {
  ...
  },
  {
    ...
    category: "Charts & Market Data",
    description: "Server-managed feed for verified candles and rule-based market intelligence.",
